...
public extension PanModalPresentable where Self: UIViewController {
    ...
    func panModalAnimate(_ animationBlock: @escaping AnimationBlockType, _ completion: AnimationCompletionType? = nil) {
        PanModalAnimator.animate(animationBlock, animationDuration: presentedVC?.presentable?.transitionDuration ?? PanModalAnimator.Defaults.defaultTransitionDuration,
                                 isDamping: true, config: self, completion)
    }
